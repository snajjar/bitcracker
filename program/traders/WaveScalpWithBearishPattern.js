            ...
            if (!this.isInTrade()) {
                ...
                if (priceDropped) {
                    if (price.spread > 0.01) {
                        ...
                        // let lastShortPeriod = 2 * 60;
                        // let lastLongPeriod = 5 * 60;

                        // // check average hourly range in the first 7 hours
                        // let longPeriodCandles = candles.slice(candles.length - lastLongPeriod, candles.length - lastShortPeriod);
                        // let hourlyRangeLong = indicators.getRange(longPeriodCandles);

                        // // check average hourly range in the last 4h
                        // let last4h = candles.slice(candles.length - lastShortPeriod);
                        // let hourlyRangeShort = indicators.getRange(last4h);

                        // if (hourlyRangeShort > hourlyRangeLong) {
                        //     return this.hold(); // too soon, there's still movement
                        // } else {
                        //     return this.buy();
                        // }

                        // console.log('First 7h=', hourlyRangePrevious7h, 'Last 4h=', hourlyRangeLast4h);
                ...
                if (!this.currentStopLoss || !this.currentTakeProfit) {
                    ...
                    this.currentStopLoss = this.currentTrade.enterPrice * (1 - this.risk + taxes);
                    this.currentTakeProfit = this.currentTrade.enterPrice * (1 + this.risk + taxes);
